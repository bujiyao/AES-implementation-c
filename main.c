...
int main()
{
	uint8_t ret_text[16] = {0};  // store decrypted data
	uint8_t text[16] = {
		0x01,0x23,0x45,0x67,     // store original data
		0x89,0xab,0xcd,0xef,
		0xfe,0xdc,0xba,0x98,
		0x76,0x54,0x32,0x10
	};
	uint8_t cipher_text[16] = {0};  // store encrypted data
	uint8_t key[32] = {             // store key
		0x0f,0x15,0x71,0xc9,
		0x47,0xd9,0xe8,0x59,
		0x0c,0xb7,0xad,0xd6,
		0xaf,0x7f,0x67,0x98,
	...
	uint32_t key_bit[3] = {128, 192, 256};  // store key bit

	aes_context ctx;    //AES context structure used to maintain the encryption and decryption states.
